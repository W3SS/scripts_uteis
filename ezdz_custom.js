...
(function(factory) {
    ...
    var defaults = {
        className: '',
        text: 'Arraste um Arquivo ou Clique',
        previewImage: true,
        value: null,
        height: null,
        width: null,
    ...
    $.ezdz = function(element, options) {
        ...
        var init = function() {
            ...
            if (settings.height) {
                $container.css('height', settings.width);
            }
            if (settings.containerCss) {
                $.each(settings.containerCss, function(css, value) {
                    $container.css(css, value)
                });
            }
    ...
    $.ezdz.prototype.generateRemoveButton = function() {
            var settings = this.settings,
                $input = this.$input,
                $ezdz = $input.parent('.' + settings.classes.main);

            var $removeButton = $("<button type='button' class='ezdz-remove'><i class='fa fa-trash'></i></button>");
            $ezdz.append($removeButton);
            $removeButton.click(function() {
                $input.ezdz('preview', null);
                settings.remove.apply($input);
            });
        }
        // Destroy ezdz
